	...
	menuTOGGLE.addEventListener(
		...
		( e ) => {
			...
			menuTOGGLE.setAttribute(
				'aria-expanded',
				'false' === menuTOGGLE.getAttribute( 'aria-expanded' ) ?
					'true' :
					'false'
			);
			document.querySelector( 'body' ).classList.toggle( 'mobile-nav-on' );
		},
		false
	);

	// Removes the nav--toggled-on class for same page menu items.
	const menuITEMS = nav.querySelectorAll( '.menu-item a' );

	// Return early if menuITEMS are missing.
	if ( ! menuITEMS ) {
		return;
	}
	...
	[].forEach.call( menuITEMS, function( menuITEM ) {
		menuITEM.addEventListener(
			...
			( e ) => {
				...
				document
					.querySelector( 'body' )
					.classList.toggle( 'mobile-nav-on' );
			},
			false
		);
	} );
